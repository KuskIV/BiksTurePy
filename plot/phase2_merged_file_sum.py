...
from sum_constructors.phase2_sum_cat_constructor import get_object as sum_cat
...
def sum_cat_file(csv_input_name, get_category, get_sub_category, get_class_accuracy, base_path):
    csv_path = f'{base_path}/{csv_input_name}' 
    
    csv_name = "final_sum_sum_sum_summed.csv"
    
    sum_path = f'{base_path}/{csv_name}'
    
    csv_obj = cvs_object(csv_path)
    data = generalized_sum(csv_obj, sum_cat(get_class_accuracy))
    csv_obj.write(data, path=sum_path, overwrite_path=True)

...
def premade_single_filter(str:str)->Filter:
    ...
    if str == 'snow':
        ...
        result = Filter({'wh_set':config})
    if str == 'snow_mild':
        config = {'density':(0.1,0.15),'density_uniformity':(0.95,1.0),'drop_size':(0.8,0.9),'drop_size_uniformity':(0.2,0.6),'angle':(-30,30),'speed':(0.1,0.15),'blur':(0.004,0.01),'mode':'snow'}
        result = Filter({'wh_set':config})
    if str == 'snow_medium':
        config = {'density':(0.2,0.27),'density_uniformity':(0.95,1.0),'drop_size':(0.8,0.9),'drop_size_uniformity':(0.2,0.6),'angle':(-30,30),'speed':(0.1,0.15),'blur':(0.004,0.01),'mode':'snow'}
        result = Filter({'wh_set':config})
    if str == 'snow_heavy':
        config = {'density':(0.35,0.45),'density_uniformity':(0.95,1.0),'drop_size':(0.8,0.9),'drop_size_uniformity':(0.2,0.6),'angle':(-30,30),'speed':(0.1,0.15),'blur':(0.004,0.01),'mode':'snow'}
        result = Filter({'wh_set':config})
    if str == 'day':
        config = {'factor':1.0}
